...
export default function MapPicker({
  ...
  useEffect(() => { onChange?.(pos || null); }, [pos, onChange]);

  const search = async (q) => {
    if (!q || q.trim().length < 3) { setResults([]); return; }
    try {
      const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&limit=5`;
      const res = await fetch(url, { headers: { "User-Agent": "construction-dashboard (demo)" } });
  ...
  return (
    <div className="w-full">
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200">
        <MapContainer center={center} zoom={pos ? 14 : 11} style={{ height }} scrollWheelZoom>
          <TileLayer
      ...
      <div className="mt-2 text-xs text-gray-600">
        {pos ? <>Selected: <span className="font-mono">{pos.lat.toFixed(5)}, {pos.lon.toFixed(5)}</span></> : <>Click on the map to select coordinates.</>}
      </div>
    </div>
  );
}
